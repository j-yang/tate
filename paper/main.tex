...
\author{
Author One\thanks{Affiliation} \and
...
when one branch deletes a parent element and another edits its child,
...
$\uparrow I = \{\text{root},\dots,\text{parent}(I), I\}$. The topology is
...
local to the ancestor chain $\uparrow I$). A \emph{global section}
...
\paragraph{The new case: delete-parent + modify-child.}
If ours deletes parent $P$ and theirs modifies $P$'s child $C$, Stage 1
gives $m_0 = \{\text{root}, C(\text{modified})\}$ with zero field conflicts
(each edit is single-sided). But $parent(C) = P$ is now absent, violating
...
(b) Witness. Let $\Id = \{\text{root}, P, C\}$, $parent(C){=}P$,
$parent(P){=}\text{root}$:
\[
B = \{\text{root}, P, C(v{=}1)\},\;
O = \{\text{root}, C(v{=}1)\},\;
T = \{\text{root}, P, C(v{=}9)\}.
\]
\emph{Discrete.} At $P$: $O(P){=}\bot, B(P){=}P, T(P){=}P$, $T{=}B$
$\Rightarrow$ take $O(P){=}\bot$ (deletion wins). At $C$: $O(C){=}B(C)$
$\Rightarrow$ take $T(C)$ (modification wins). No field is double-changed
$\Rightarrow$ $C_{\text{disc}} = \emptyset$. But the result
$\{\text{root}, C(v{=}9)\}$ has $parent(C){=}P$ absent: not a section.
\emph{Sheaf.} $\Sigma$ drops $C$, recording \texttt{Dangling}@$C$;
$C_{\text{sheaf}} \neq \emptyset$.
\end{proof}

\begin{remark}
The witness is the smallest delete-parent / modify-child scenario. The
class of such witnesses is exactly the support of $\Sigma$'s correction
--- non-empty iff the tree has depth $\geq 1$ and the two branches' edits
cross an ancestry edge inconsistently. On a flat (discrete) tree,
$\Sigma = \mathrm{id}$ and Theorem~\ref{thm:strict} collapses to
equality: the discrete merge \emph{is} the sheaf merge in that degenerate
case.
\end{remark}
...
(v2.0.0, MIT license, available on crates.io).
...
\texttt{dangling\_parent\_after\_delete\_modify\_is\_structural\_conflict}
instantiates the theorem's witness ($\Id = \{\text{root}, P, C\}$; ours
deletes $P$, theirs modifies $C$) and asserts (i) the sheaf merge reports
a \texttt{Dangling} conflict at $C$ with \texttt{missing\_parent = P}, and
(ii) $C$ is dropped from the merged section. The same input under the
